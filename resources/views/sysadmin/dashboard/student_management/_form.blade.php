...
<div class="space-y-6">
    ...
    <div>
        <label for="password" class="block text-sm font-medium text-secondary mb-2">Password @if(isset($student)) <span class="text-xs font-normal text-secondary/70">(Kosongkan jika tidak ingin mengubah)</span> @endif</label>
    ...
    <div class="flex items-center gap-3 pt-4 border-t border-border">
        ...
        <a href="{{ route('sysadmin.manage_student.index') }}" class="px-6 py-2.5 bg-surface border border-border text-secondary font-medium rounded-lg hover:text-primary hover:border-primary transition-colors focus:ring-2 focus:ring-border">
            Batal
        </a>
    </div>
</div>
