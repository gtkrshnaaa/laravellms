...
@section('content')
    ...
    <h1 class="text-2xl font-bold text-dark-text mb-4">Edit Video: {{ $video->title }}</h1>
    <div class="bg-white p-6 rounded-lg border-2 border-gray-100">
        <form action="{{ route('course_admin.management.topics.videos.update', [$topic, $video]) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" class="w-full border border-gray-300 px-3 py-2 rounded-md shadow-sm focus:border-main-red focus:ring-main-red @error('title') border-gray-500 @enderror" value="{{ old('title', $video->title ?? '') }}" required>
            ...
            <div class="mb-4">
                ...
                <input type="url" name="youtube_url" id="youtube_url" class="w-full border border-gray-300 px-3 py-2 rounded-md shadow-sm focus:border-main-red focus:ring-main-red @error('youtube_url') border-gray-500 @enderror" value="{{ old('youtube_url', $video->youtube_url ?? '') }}" required>
            ...
            <div class="mb-6">
                ...
                <input type="number" name="order" id="order" class="w-full border border-gray-300 px-3 py-2 rounded-md shadow-sm focus:border-main-red focus:ring-main-red @error('order') border-gray-500 @enderror" value="{{ old('order', $video->order ?? '0') }}" required>
