...
@section('content')
    ...
    <h1 class="text-2xl font-bold text-black mb-4">Edit Kuis: {{ $quiz->title }}</h1>
    <div class="bg-white p-6 rounded-lg border border-blue-100">
        <form action="{{ route('course_admin.management.topics.quizzes.update', [$topic, $quiz]) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" class="w-full border border-blue-100 px-3 py-2 rounded-md shadow-sm focus:border-blue-500 @error('title') border-blue-500 @enderror" value="{{ old('title', $quiz->title ?? '') }}" required>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" class="w-full border border-blue-100 px-3 py-2 rounded-md shadow-sm focus:border-blue-500 @error('description') border-blue-500 @enderror">{{ old('description', $quiz->description ?? '') }}</textarea>
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <input type="number" name="min_score" id="min_score" class="w-full border border-blue-100 px-3 py-2 rounded-md shadow-sm focus:border-blue-500 @error('min_score') border-blue-500 @enderror" value="{{ old('min_score', $quiz->min_score ?? '70') }}" required>
                ...
                <div>
                    ...
                    <input type="number" name="order" id="order" class="w-full border border-blue-100 px-3 py-2 rounded-md shadow-sm focus:border-blue-500 @error('order') border-blue-500 @enderror" value="{{ old('order', $quiz->order ?? '0') }}" required>
