...
@section('content')
    ...
    <h1 class="text-2xl font-bold text-dark-text mb-4">Tambah Kuis untuk Topik: {{ $topic->title }}</h1>
    <div class="bg-white p-6 rounded-lg border-2 border-gray-100">
        <form action="{{ route('course_admin.management.topics.quizzes.store', $topic) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" class="w-full border border-gray-300 px-3 py-2 rounded-md shadow-sm focus:border-main-red focus:ring-main-red @error('description') border-gray-500 @enderror">{{ old('description') }}</textarea>
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <input type="number" name="min_score" id="min_score" class="w-full border border-gray-300 px-3 py-2 rounded-md shadow-sm focus:border-main-red focus:ring-main-red @error('min_score') border-gray-500 @enderror" value="{{ old('min_score', '70') }}" required>
